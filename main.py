...
# --- [비즈니스 로직 함수] ---
def calculate_metrics(weight: float, height: float, systolic: int, diastolic: int, blood_sugar: int, steps: int, sleep_hours: float):
    height_m = height / 100
    bmi = round(weight / (height_m * height_m), 1)

    bmi_cat = "저체중" if bmi < 18.5 else ("정상" if bmi <= 22.9 else ("과체중" if bmi <= 24.9 else "비만"))
    bp_cat = "정상" if (systolic < 120 and diastolic < 80) else ("고혈압" if (systolic >= 140 or diastolic >= 90) else "주의")
    sugar_cat = "정상" if blood_sugar < 100 else ("공복혈당장애" if blood_sugar <= 125 else "당뇨 의심")
    act_cat = "부족" if steps < 5000 else ("적정" if steps < 10000 else "우수")
    sleep_cat = "수면 부족" if sleep_hours < 7.0 else ("적정 수면" if sleep_hours <= 9.0 else "수면 과다")

    return {
        "bmi": bmi, "bmi_category": bmi_cat,
        "bp_category": bp_cat, "sugar_category": sugar_cat,
        "activity_category": act_cat, "sleep_category": sleep_cat
    }
...
# --- [건강 기록 CRUD 엔드포인트 (JWT 인증 필요)] ---
@app.post("/records", response_model=RecordOut)
def create_record(record: RecordIn, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    metrics = calculate_metrics(db_record.weight, db_record.height, db_record.systolic, db_record.diastolic, db_record.blood_sugar, db_record.steps, db_record.sleep_hours)
    for k, v in metrics.items():
        setattr(res, k, v)
    return res

@app.get("/records", response_model=List[RecordOut])
def get_records(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    records = db.query(models.Record).filter(models.Record.user_id == current_user.id).all()
    results = []
    for r in records:
        r_out = RecordOut.model_validate(r)
        metrics = calculate_metrics(r.weight, r.height, r.systolic, r.diastolic, r.blood_sugar, r.steps, r.sleep_hours)
        for k, v in metrics.items():
            setattr(r_out, k, v)
...
def get_dashboard():
    ...
    <body>
        ...
        <div id="authSection" class="card">
            ...
            <button onclick="signup()" style="background: #28a745;">회원가입</button>
        </div>
        ...
        <div id="appSection" class="hidden">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h3 id="welcomeMsg"></h3>
                <button onclick="logout()" style="background: #dc3545;">로그아웃</button>
            </div>
        ...
        <script>
            ...
            async function signup() {
                ...
                alert(res.ok ? data.message : '회원가입 실패: ' + data.detail);
            }
